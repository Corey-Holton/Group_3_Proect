...
warnings.filterwarnings(
    "ignore", category=UserWarning, message="Coremltools is not installed."
)
warnings.filterwarnings(
    "ignore", category=UserWarning, message="tflite-runtime is not installed."
)
warnings.filterwarnings(
    "ignore",
...
def process_audio_stems(
    ...
    if results is None:
        return print_line(
            "[ERROR] No results returned from `separate_audio`.", text_color="red"
        )

    other, vocal, bass, drums = results
    return str(other), str(vocal), str(bass), str(drums)
...
def translate_audio_lyrics(lyrics, language_code):
    ...
    translated = " ".join(
        extract_lyrics.translate_lyrics(lyrics.split("\n"), language_code)
    )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                audio_input = gr.Audio(
                    type="filepath", label="Upload Audio File", sources="upload"
                )
            ...
            with gr.Column(scale=1):
                ...
                model = gr.Textbox(
                    value="htdemucs_ft", label="Demucs Model", placeholder="htdemucs_ft"
                )
    ...
    with gr.Blocks() as interface:
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                generate_audio_from_midi = gr.Checkbox(
                    label="Sonify MIDI?", value=False
                )
                save_model_outputs = gr.Checkbox(
                    label="Save Model Outputs?", value=False
                )
                ...
                allow_multiple_pitch_bends = gr.Checkbox(
                    label="Allow Multiple Pitch Bends?", value=False
                )
                apply_melodia_trick = gr.Checkbox(
                    label="Apply Melodia Trick?", value=True
                )
    ...
    with gr.Blocks() as interface:
        with gr.Row():
            with gr.Column(scale=1):
                ...
                midi_input = gr.Audio(
                    type="filepath", label="Upload MIDI File", sources="upload"
                )
    ...
    with gr.Blocks(theme="shivi/calm_seafoam") as lyrics_interface:
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                language = gr.Textbox(label="Language Code", placeholder="es")
            ...
            do_translate.click(
                ...
                inputs=[lyrics, language],
                outputs=[translated_lyrics],
            )
    return lyrics_interface
